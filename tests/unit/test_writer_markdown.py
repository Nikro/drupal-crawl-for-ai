...
def test_write_returns_correct_path(tmp_path) -> None:
...
def test_write_creates_intermediate_dirs(tmp_path) -> None:
    writer = MarkdownWriter(tmp_path)
    writer.write("deep/nested/type", 1, "Content")
    assert (tmp_path / "deep" / "nested" / "type" / "1.md").exists()


def test_overwrite_is_idempotent(tmp_path) -> None:
    writer = MarkdownWriter(tmp_path)
...
def test_content_written_correctly(tmp_path) -> None:
    writer = MarkdownWriter(tmp_path)
    content = "# Hello\n\nThis is the body."
